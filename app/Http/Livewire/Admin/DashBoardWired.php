...
class DashBoardWired extends Component
{
    ...
    public function get_order_monthly_rate_diff($column_name)
    {
        ...
        // Last month's record
        $lastMonthRecord = $query->whereBetween('created_at', [$startOfLastMonth, $endOfLastMonth])->sum($column_name);

        // Reset the query for this month's record
        $query = $admin->type == 'superadmin' ? OrdersModel::query() : OrdersModel::where('issued_by', $admin->id);
        $thisMonthRecord = $query->whereBetween('created_at', [$startOfThisMonth, $endOfThisMonth])->sum($column_name);
    ...
    public function get_annual_record($column)
    {
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            // Loop through the months and calculate the sum for each
            $final_monthy_records[$month] = $query->whereBetween('created_at', [$start_date, $end_date])->sum($column);
    ...
    public function render()
    {
        ...
        // Query today's sales and format the result
        $this->today_sell = $this->number_to_kmb(
            $query->where('order_date', $today)->sum('total')
        );

        $this->sell_pct_change = $this->get_order_monthly_rate_diff('total');


        $this->today_due = $this->number_to_kmb($query->where('order_date', $today)->sum('due'));
        $this->due_pct_change = $this->get_order_monthly_rate_diff('due');

        $this->monthly_income_records = $this->get_annual_record('pay');
        $this->today_income = $this->number_to_kmb(
            $query->where('order_date', $today)->sum('pay')
